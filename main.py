...
# xls_df = pd.read_excel(xls_path, sheet_name="MD5") #SAMPLE_NAME #ENA_FILE_PATH MD5SUM
# down = []
# with open(ok_path, "r") as f,open("download.txt","w") as f_out:
#     lines = f.readlines()
#     for line in lines:
#         line = line.replace("\n", "")
#         row = xls_df[xls_df['SAMPLE_NAME'] == line]
#         if not row.empty:
#             ftp_path = row.iloc[0]['#ENA_FILE_PATH'] + ".crai" + "\n"
#             down.append(ftp_path)
#             # print(ftp_path)
#     f_out.writelines(down)
#
#
# f.close()
# f_out.close()
